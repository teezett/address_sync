...
= Software Requirements Specification for Address-Synchronization

This document describes the requirements of <<index.adoc#,project Address-Synchronization>>.
...
[cols=">.^1s,<10"]
|====
|Goal | All or one named reminder(s) shall be started once
|Scope | system
|Level | user
|Preconditions | the configuration is valid (<<usecase-read-config,UseCase "read configuration">>)
|Success condition | the reminder(s) are executed once and the program exits
|Failure condition | program exit with failure
...
--
start
if (a named reminder is selected
to be started immediately?) then (yes)
    if (a reminder
with the given name
is configured?) then (yes)
        :create a reminder job
        triggered immidiatly
        for selected reminder;
    else (no)
        :log error message;
        end
    endif
else (no)
    while (for each configured reminder) is (next)
        if (no cron trigger defined
or single execution advised?) then (yes)
            :create a reminder job
            triggered immidiatly;
        else (no)
            :create a reminder job
            triggered regularly as defined;
        endif
    endwhile (done)
endif
stop
